...
const CFG = {
    ...
    STATS: {
        // REBALANCED CLASSES
        rock: { spd: 7.5, hp: 200, dash: 22, skillCd: 280, skillDur: 20, passive: 'thorns' },
        paper: { spd: 8.8, hp: 140, dash: 25, skillCd: 400, skillDur: 90, passive: 'stealth' },
        scissors: { spd: 8.2, hp: 150, dash: 23, skillCd: 300, skillDur: 10, passive: 'lifesteal' },
        zombie: { spd: 9.0, hp: 50, dash: 0, skillCd: 0, skillDur: 0, passive: 'aggro' }
    },
    PERKS: {
        // 8 BALANCED PERKS
        vamp: { n: 'Wampiryzm', d: 'Leczenie za 15% obrażeń' },
        glass: { n: 'Szklane Działo', d: '+40% DMG, -25% HP' },
        tank: { n: 'Pancerz', d: '+40% HP, -15% Speed' },
        speed: { n: 'Nitro', d: '+15% Speed, +10% Dash' },
        regen: { n: 'Regeneracja', d: '+5 HP co 2 sek' },
        crit: { n: 'Krytyk', d: '25% szans na 2x DMG' },
        dodge: { n: 'Unik', d: '15% szans uniknięcia DMG' },
        magnet: { n: 'Magnes', d: '+50% zasięg zbierania orb' }
    },
    R_ZONE: 180,
    MAGNET_RANGE: 200
};
...
class Entity {
    ...
    takeDamage(amt, source) {
        ...
        // DODGE PERK: 15% chance to avoid damage
        if (this instanceof Player && this.perks.includes('dodge') && Math.random() < 0.15) {
            io.to(this.roomId).emit('fx', { t: 'dmg', x: this.x, y: this.y - 20, v: 'UNIK!', c: '#00ffff' });
            return false;
        }
...
class Player extends Entity {
    ...
    applyStats() {
        ...
        this.baseDash = base.dash;
        this.baseSpd = base.spd;

        // Apply perk modifiers
        if (this.perks.includes('tank')) { this.maxHp *= 1.4; this.baseSpd *= 0.85; }
        if (this.perks.includes('glass')) { this.maxHp *= 0.75; }
        if (this.perks.includes('speed')) { this.baseSpd *= 1.15; this.baseDash *= 1.1; }
        if (this.perks.includes('magnet')) { this.magnetRange = CFG.MAGNET_RANGE * 1.5; } else { this.magnetRange = CFG.MAGNET_RANGE; }
    ...
    applyPerk(pid) {
        ...
        if (this.perks.length >= 4) return; // Max 4 perks
        if (this.perks.includes(pid)) return; // No duplicates
    ...
    update(room) {
        ...
        if (this.stun > 0) {
            ...
            // Paper passive: slow regen
            if (this.type === 'paper' && this.hp < this.maxHp && room.ticks % 30 === 0) this.hp += 3;

            // REGEN PERK: +5 HP every 2 seconds (60 ticks)
            if (this.perks.includes('regen') && this.hp < this.maxHp && room.ticks % 60 === 0) {
                this.hp = Math.min(this.maxHp, this.hp + 5);
                io.to(room.id).emit('fx', { t: 'dmg', x: this.x, y: this.y, v: '+5', c: '#00ff88' });
            }
            ...
            if (this.input.d && this.cdDash <= 0) {
                this.cdDash = 60;
                spd = this.baseDash || CFG.STATS[this.type].dash;
...
class Room {
    ...
    handleCollisions() {
        Object.values(this.players).filter(p => p.active && !p.dead).forEach(p => {
            const magnetRange = p.magnetRange || CFG.MAGNET_RANGE;
    ...
    resolveCombat(winner, loser) {
        ...
        if (winner instanceof Player) {
            // Glass Cannon: +40% damage
            if (winner.perks.includes('glass')) dmg *= 1.4;

            // Crit from perk (25%) OR scissors passive (20%)
            const critChance = winner.perks.includes('crit') ? 0.25 : (winner.type === 'scissors' ? 0.20 : 0);
            if (Math.random() < critChance) { dmg *= 2; isCrit = true; }

            // Vampirism: 15% lifesteal
            if (winner.perks.includes('vamp')) {
                const heal = dmg * 0.15;
                winner.hp = Math.min(winner.maxHp, winner.hp + heal);
                io.to(this.id).emit('fx', { t: 'dmg', x: winner.x, y: winner.y, v: '+' + Math.round(heal), c: '#00ff00' });
            }
        }
        const tookDamage = loser.takeDamage(dmg, winner);
        if (tookDamage) {
            const color = isCrit ? '#ffd700' : '#fff';
            const displayDmg = isCrit ? Math.round(dmg) + '!' : Math.round(dmg);
            io.to(this.id).emit('fx', { t: 'dmg', x: loser.x, y: loser.y, v: displayDmg, c: color });
